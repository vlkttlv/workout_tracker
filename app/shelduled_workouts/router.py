...
from app.exceptions import (
    IncorrectIDOfShelduledWorkout,
    IncorrectIDOfWorkoutPlan,
    InvalidWorkoutStatusValue,
)
...
@router.post("/", status_code=201, summary="Creates a shelduled workout")
async def add_shelduled_workout(shelduled_workout: AddShelduledWorkout,
    ...
    workout_plan = await WorkoutPlansDAO.find_by_id(
        workout_plan_id=shelduled_workout.workout_plan_id, user_id=current_user.id
    )
    if workout_plan == []:
        raise IncorrectIDOfWorkoutPlan
    await ShelduledWorkoutDAO.add(
...
async def get_shelduled_workouts(status: str,
    order_by_time: str = Query(description="""Сортировка по возрастанию: asc, по убыванию: desc"""),
    current_user=Depends(get_current_user)):
    """Возвращает все запланированные тренировки пользователя"""
    res = await ShelduledWorkoutDAO.find_all(
        asc_or_desc=order_by_time, user_id=current_user.id, status=status
    )
    return res
...
async def get_shelduled_workout(shelduled_workout_id: int,
    ...
    res = await ShelduledWorkoutDAO.find_one_or_none(
        user_id=current_user.id, id=shelduled_workout_id
    )
    return res
...
async def update_scheduled_workout(shelduled_workout_id: int,
    ...
    if shelduled_workout.shelduled_date is not None:
        await ShelduledWorkoutDAO.update(
            id=shelduled_workout_id, shelduled_date=shelduled_workout.shelduled_date
        )
    if shelduled_workout.shelduled_time is not None:
        await ShelduledWorkoutDAO.update(
            id=shelduled_workout_id, shelduled_time=shelduled_workout.shelduled_time
        )
...
async def update_status(shelduled_workout_id: int,
    ...
    if status in ["done", "skip", "in progress"]:
        await ShelduledWorkoutDAO.update(id=shelduled_workout_id, status=status)
    else:
